...
let penaltyPool = JSON.parse(localStorage.getItem('hisab_penalties')) || [{ title: "30 Min Intense Focus Workout", points: 50 }];
...
function toggleNotifications() {
    ...
    if (Notification.permission === "granted") {
        alert("Notifications are already enabled! You will receive alerts for pending tasks.");
        checkAndNotify();
        ...
        alert("You have blocked notifications for this app. Please enable them in your device settings.");
    }
}
...
function checkAndNotify() {
    ...
    tasks.forEach(t => {
        if (t.isPenalty) hasPenalty = true;
        else if (t.type !== 'bad' && !t.isCompleted && t.currentTarget > 0) {
            pendingCount++;
        }
    });

    if (hasPenalty) {
        new Notification("Hisab Warning! ⚠️", {
            body: "You have an active Penalty Task that needs to be completed.",
            icon: "https://via.placeholder.com/192/cf6679/ffffff?text=!"
        });
    } else if (pendingCount > 0) {
        new Notification("Hisab Reminder 📝", {
            body: `You have ${pendingCount} tasks pending. Keep your streak alive!`,
            icon: "https://via.placeholder.com/192/03dac6/000000?text=+"
        });
    }
}

// Check for notifications every 3 hours if the app is open
setInterval(checkAndNotify, 1000 * 60 * 60 * 3);


// ==========================================
// UI TABS LOGIC & TOGGLES
// ==========================================
    ...
    categories.forEach(cat => {
        ...
        filteredTasks.forEach(task => {
            ...
            if (task.type === 'bad') {
                ...
                div.innerHTML = `
                    <div class="task-header"><div class="task-title-area"><strong>${task.title}</strong></div>
                    <div class="task-controls"><button class="btn-delete" onclick="deleteTask('${task.id}')">🗑️</button></div></div>
                    <div class="task-status">Penalty: -${task.pointsPerUnit} pts if triggered</div>
                    <button class="btn-fail" onclick="punish('${task.id}', '${task.title}')">I Messed Up</button>
                `;
            } else {
                ...
                let timeInfo = '';
                if (task.type === 'fixed-period') timeInfo = `<div style="font-size:0.75rem; color:#aaa; margin-bottom:5px;">Window: ${task.startDate} to ${task.endDate}</div>`;
                if (task.type === 'one-time') timeInfo = `<div style="font-size:0.75rem; color:#aaa; margin-bottom:5px;">One-Time Objective</div>`;

                let statusHTML = isAhead 
                    ? `<span class="highlight-banked">✅ Done! Credit banked: ${Math.abs(task.currentTarget)} units.</span>` 
                    : `⏳ Pending: <strong>${task.currentTarget}</strong> (Base: ${task.baseTarget})`;

                div.innerHTML = `
                    <div class="task-header">
                        <div class="task-title-area"><strong>${task.isPenalty ? '⚠️ ' : ''}${task.title}</strong><span style="font-size:0.8rem; color:var(--primary);">+${task.pointsPerUnit} pts/unit</span></div>
                        <div class="task-controls"><button class="btn-edit" onclick="editTask('${task.id}')">✏️</button><button class="btn-delete" onclick="deleteTask('${task.id}')">🗑️</button></div>
                    </div>
                    ${timeInfo}
                    <div class="task-status">${statusHTML}</div>
                    <div class="task-action">
                        <input type="number" id="input-${task.id}" value="1" min="1"><button class="btn-done" onclick="logProgress('${task.id}')">Log Units</button>
                    </div>
                `;
            }
...
function saveTask() {
    ...
    const type = document.getElementById('task-type').value;
    const baseTarget = parseInt(document.getElementById('task-base-target').value) || 1;
    ...
    if (id) {
        ...
        task.pointsPerUnit = pointsPerUnit;
        task.startDate = startDate;
        task.endDate = endDate;
    } else {
        tasks.push({ 
            id: Date.now().toString(), title, type, 
            baseTarget, currentTarget: baseTarget, 
            pointsPerUnit, startDate, endDate,
            lastChecked: new Date().toISOString(), isPenalty: false, isCompleted: false
        });
    }
...
function editTask(id) {
    ...
    document.getElementById('btn-save-task').innerText = 'Save Changes';
    document.getElementById('btn-cancel-edit').style.display = 'inline-block';
    
    document.getElementById('task-id').value = task.id; 
    document.getElementById('task-title').value = task.title;
    document.getElementById('task-type').value = task.type || 'daily';
    document.getElementById('task-base-target').value = task.baseTarget; 
    document.getElementById('task-points-unit').value = task.pointsPerUnit;
...
function cancelEdit() {
    ...
    document.getElementById('btn-save-task').innerText = 'Add Task';
...
function deleteTask(id) {
    if (confirm("Are you sure you want to delete this task?")) { tasks = tasks.filter(t => t.id !== id); saveData(); render(); }
}

// ==========================================
// CORE LOGIC: LOGGING & CUMULATIVE ROLLOVER
// ==========================================
function logProgress(id) {
    const task = tasks.find(t => t.id === id);
    const amountDone = parseInt(document.getElementById(`input-${id}`).value) || 0;
    if (amountDone <= 0) return;
    
    const pointsEarned = amountDone * task.pointsPerUnit;
    score += pointsEarned;
    ...
    if (task.isPenalty) {
        ...
        if (task.type === 'one-time' && task.currentTarget <= 0) {
...
function punish(id, title) {
    ...
    pointHistory.push({ timestamp: Date.now(), points: -task.pointsPerUnit, title: "Failed: " + title });
    saveData(); render();
}
...
function processRollovers() {
    ...
    tasks.forEach(task => {
        ...
        if (task.type === 'daily') {
            task.currentTarget += (task.baseTarget * diffDays);
        }
        else if (task.type === 'weekly') {
            const weeksPassed = Math.floor(diffDays / 7);
            if (weeksPassed >= 1) task.currentTarget += (task.baseTarget * weeksPassed);
        }
            ...
            if (monthsPassed >= 1) task.currentTarget += (task.baseTarget * monthsPassed);
        }
            ...
            for (let i = 1; i <= diffDays; i++) {
                let checkDate = lastCheckedMidnight + (i * 86400000);
                if (checkDate >= start && checkDate <= end) validDays++;
            }
...
function updateDashboard() {
    ...
    progressChart = new Chart(ctx, {
        data: {
            ...
            datasets: [
                { type: 'line', label: 'Min to Avoid Penalty', data: [50, 50, 50, 50], borderColor: '#f6e58d', borderWidth: 3, borderDash: [5, 5], fill: false },
...
function evaluatePerformance() {
    ...
    if (currentWeek !== lastEvals.week) {
        ...
        if (lastWeekScore < dynTargets.weekly.min) {
            ...
            tasks.push({ 
                id: Date.now().toString() + "-pen", title: selectedPenalty.title, 
                type: 'daily', baseTarget: 1, currentTarget: 1, pointsPerUnit: selectedPenalty.points, 
                lastChecked: new Date().toISOString(), isPenalty: true 
            });
...
function renderPool() {
    ...
    penaltyPool.forEach((p, index) => {
        const div = document.createElement('div');
        div.style = "display:flex; justify-content:space-between; align-items:center; background:#3a1c1e; padding:8px; border-radius:5px; margin-top:5px;";
        div.innerHTML = `<span>${p.title} (-${p.points})</span><button onclick="removePoolItem(${index})" style="background:transparent; color:var(--danger); margin:0; width:auto; padding:0 10px;">❌</button>`;
        list.appendChild(div);
    });
}
function addToPool() {
    const title = document.getElementById('pool-title').value;
    const points = parseInt(document.getElementById('pool-points').value) || 50;
    if (!title) return alert("Please enter a punishment description.");
    penaltyPool.push({ title, points });
    document.getElementById('pool-title').value = ''; document.getElementById('pool-points').value = '';
    saveData(); renderPool();
}
...
function handleDriveAction(action) {
    ...
    tokenClient.callback = async (resp) => {
        ...
        try {
            ...
            if (action === 'backup') {
                const fileContent = JSON.stringify({ tasks, score, history: pointHistory, evals: lastEvals, penalties: penaltyPool });
...
evaluatePerformance();
render();
// Fire a notification check shortly after startup
